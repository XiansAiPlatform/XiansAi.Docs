...
Call `SendFileAsync` (or `ReplyAsync` with a file) from a **message handler or activity** — `OnUserChatMessage`, `OnFileUpload`, and so on. Do **not** call it from workflow code: file bytes cannot pass through Temporal payloads.
...
|--------|------------------------|
| `ReplyAsync(text, file)` or `SendFileAsync(..., text: "...")` | **One** agent bubble: caption + filename + download icon |
...
!!! note "Not on proactive messaging"
    `XiansContext.Messaging` (`SendChatAsync` / `SendDataAsync`) does not send files. File send is handler/activity-only via `UserMessageContext`.
...
| Send (agent → user) | `context.SendFileAsync(...)` or `context.ReplyAsync(text, file)` from a handler/activity |
